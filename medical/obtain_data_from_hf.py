...
def parse_medical_o1_verifiable_problem(dataset: datasets.Dataset) -> Tuple[List[dict], str]:
    reward_type = "match"
    res_data = []
    for item in tqdm(dataset):
        if len(item["Open-ended Verifiable Question"].strip()) == 0 or len(item["Ground-True Answer"]) == 0:
            continue
        res_data.append({
            "question": item["Open-ended Verifiable Question"].strip(),
            "answer": item["Ground-True Answer"].strip(),
        })
    res_data = [  # filter out long-answer questions. 40228-> 35401(5) 34065(4) 31943(3) 28791(2) 22870(1) 10470(0)
        item for item in res_data
        if item["answer"].strip().count(" ") <= 2
    ]
    return res_data, reward_type
...
DATA_INFO_FUNCS = {
    "FreedomIntelligence/medical-o1-verifiable-problem": parse_medical_o1_verifiable_problem,
}
